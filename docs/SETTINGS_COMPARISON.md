...
This document tracks behavior shared by the upstream React Native mobile client and this native Kotlin Android client. The native UI follows the same product behavior while using Compose and Android-native navigation.

## Current matrix

| Setting                    | Native Android behavior                                                                                                            | Parity note                                                                                                        |
| -------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| T3 Account                 | Clerk email-code and OAuth sign-in, account state, sign-out                                                                        | Client path is implemented; production OAuth still requires the native Android redirect to be allowlisted in Clerk |
...
| T3 Connect environments    | Relay section with refresh, connect/disconnect switches, saved-relay fallback, and discovery errors within the Environments screen | Uses the native Clerk, DPoP, and relay client; end-to-end OAuth acceptance remains externally gated                |
...
The consolidated automated and manual acceptance record is in [`RELEASE_READINESS.md`](RELEASE_READINESS.md).
